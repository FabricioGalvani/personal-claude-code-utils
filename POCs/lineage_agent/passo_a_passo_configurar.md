Aqui vai tudo consolidado — as duas versões finais dos arquivos (com tudo que discutimos sobre aliases e propagação de origem) e o passo a passo.

## Arquivo 1 — `.github/chatmodes/lineage.chatmode.md`
...
## Arquivo 2 — `.github/prompts/lineage.prompt.md`
...
## Passo a passo — configurar no VS Code

**1. Pré-requisitos**

Tenha a extensão **GitHub Copilot** e **GitHub Copilot Chat** instaladas e atualizadas no VS Code. Os custom chat modes são uma feature relativamente recente — se você não vê o seletor de modo no painel do chat, atualize a extensão.

**2. Crie a estrutura de pastas no repo**

Na raiz do projeto onde estão seus scripts de ETL (ou em qualquer projeto onde queira usar o agente), crie:
...
Cola o conteúdo dos dois arquivos acima nos respectivos caminhos.

**3. Recarregue o VS Code**

Use o comando `Developer: Reload Window` (`Ctrl/Cmd + Shift + P` → digita "Reload Window"). Isso força o Copilot a reler os arquivos de configuração.

**4. Abra o Copilot Chat e selecione o modo**

Abra o painel do Copilot Chat (`Ctrl/Cmd + Alt + I` geralmente). No topo do painel tem um seletor de modo — troque para **lineage** (o nome vem do campo `description` do `.chatmode.md`).

**5. Adicione os scripts ao contexto**

Antes de pedir a análise, dá pro Copilot os arquivos que ele deve analisar. Três formas que funcionam bem:

- Digita `#file:` e seleciona cada script do seu ETL.
- Arrasta os arquivos do explorador direto pro chat.
- Pra pegar uma pasta inteira: `#folder:caminho/da/pasta_etl`.

**6. Dispare a análise**

Digita `/lineage` no chat. Ele vai pedir o nome da tabela de destino (o `${input:tabela_destino:...}` do prompt file). Responde e ele começa a análise pela Fase 1.

**7. Estratégia recomendada pra ETLs grandes**

Se o ETL tem muitos scripts/transformações, peça em duas levas: primeiro **só Fase 1 e 2**, confere o glossário de origem e o grafo, corrige se ele entendeu algum alias errado, e só depois pede **Fase 3 e 4**. Isso reduz drasticamente erros de linhagem e fica mais barato em tokens. Pra isso, na primeira mensagem é só completar com "Pare ao final da Fase 2 e aguarde minha revisão antes da Fase 3".

## Verificação rápida de que tá funcionando

Depois de configurar, se você abrir o seletor de modos no chat e **lineage** aparecer na lista, tá tudo certo. Se o `/lineage` for autocompletado quando você começa a digitar, o prompt file também tá carregado.

Se algo não aparecer, os pontos mais comuns de falha são: extensão desatualizada, pasta `.github` na raiz errada (precisa ser na raiz do workspace aberto no VS Code, não num subdiretório), ou não ter feito o reload da janela.

Quando rodar pela primeira vez num ETL seu, manda o output (ou um pedaço dele) que eu te ajudo a ajustar o prompt se algum ponto sair confuso.